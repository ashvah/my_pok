...
uint8_t pok_sched_remaining_slot = POK_CONFIG_SCHEDULING_NBSLOTS;

void (*sched_global_func)(void);
...
void pok_sched_init(void) {
  ...
  pok_sched_current_slot = POK_CONFIG_SCHEDULING_NBSLOTS - 1;
  pok_sched_next_major_frame = POK_CONFIG_SCHEDULING_MAJOR_FRAME;
  pok_sched_next_deadline = 0;
  pok_sched_next_flush = 0;
  pok_current_partition = pok_sched_slots_allocation[0];
  pok_sched_remaining_slot = POK_CONFIG_SCHEDULING_NBSLOTS;
#ifdef POK_NEEDS_SCHED_MLFQ
  pok_sched_queue_initialize();
#endif
#if POK_CONFIG_NB_PARTITIONS > 1
  pok_partition_setup_global_scheduler();
  sched_global_func();
...
#if POK_CONFIG_NB_PARTITIONS > 1
void pok_partition_setup_global_scheduler() {
#ifdef POK_CONFIG_GLOBAL_SCHEDULER
  switch (POK_CONFIG_GLOBAL_SCHEDULER) {
#ifdef POK_NEEDS_SCHED_GLOBAL_PRIORITY
  case POK_SCHED_GLOBAL_PRIORITY:
    sched_global_func = &pok_sched_global_priority;
    break;
#endif // POK_NEEDS_GLOBAL_PRIORITY
#ifdef POK_NEEDS_SCHED_GLOBAL_EDF
  case POK_SCHED_GLOBAL_EDF:
    sched_global_func = &pok_sched_global_edf;
    break;
#endif // POK_NEEDS_SCHED_GLOABL_EDF
#ifdef POK_NEEDS_SCHED_GLOBAL_WRR
  case POK_SCHED_GLOBAL_WRR:
    sched_global_func = &pok_sched_global_wrr;
    break;
#endif // POK_NEEDS_SCHED_GLOBAL_WRR

  default:
    sched_global_func = &pok_sched_global_rr;
    break;
  }
#else
  sched_global_func = &pok_sched_global_rr;
...
void pok_sched_global_rr() {
  ...
  if (pok_sched_remaining_slot > 1) {
    pok_sched_remaining_slot -= 1;
  } else {
    pok_sched_remaining_slot = POK_CONFIG_SCHEDULING_NBSLOTS;
  }

#ifdef POK_NEEDS_DEBUG
  printf("select slot %d, next deadline %lld, number of remaining slots: %d\n",
         pok_sched_current_slot, pok_sched_next_deadline,
         pok_sched_remaining_slot);
#endif
}

#ifdef POK_NEEDS_SCHED_GLOBAL_PRIORITY
uint8_t pok_sched_slots_priority[POK_CONFIG_SCHEDULING_NBSLOTS] =
    (uint8_t[])POK_CONFIG_SCHEDULING_SLOTS_PRIORITY;
uint8_t pok_sched_slots_state[POK_CONFIG_SCHEDULING_NBSLOTS];

void pok_sched_global_priority() {
  if (pok_sched_remaining_slot == POK_CONFIG_SCHEDULING_NBSLOTS) {
    for (uint8_t slot = 0; slot < POK_CONFIG_SCHEDULING_NBSLOTS; ++slot) {
      pok_sched_slots_state[slot] = 1;
    }
  }
  ...
  uint8_t max_slot = 0;
  for (uint8_t slot = 0; slot < POK_CONFIG_SCHEDULING_NBSLOTS; ++slot) {
    if (pok_sched_slots_state[slot] == 1 &&
        pok_sched_slots_priority[slot] > max_prio) {
      max_prio = pok_sched_slots_priority[slot];
      max_slot = slot;
    }
  }

  pok_sched_current_slot = max_slot;
  pok_sched_next_deadline += pok_sched_slots[pok_sched_current_slot];

  pok_sched_slots_state[max_slot] = 0;
...
#ifdef POK_NEEDS_DEBUG
  printf("select slot %d, next deadline %lld, priority %d, number of remaining "
         "slots: %d\n",
         pok_sched_current_slot, pok_sched_next_deadline,
         pok_sched_slots_priority[max_slot], pok_sched_remaining_slot);
#endif
}
#endif

#ifdef POK_NEEDS_SCHED_GLOBAL_EDF
int64_t pok_sched_slots_deadline[POK_CONFIG_SCHEDULING_NBSLOTS] =
    (int64_t[])POK_CONFIG_SCHEDULING_SLOTS_DEADLINE;
int64_t pok_sched_slots_abs_deadline[POK_CONFIG_SCHEDULING_NBSLOTS] =
    (int64_t[])POK_CONFIG_SCHEDULING_SLOTS_DEADLINE;

void pok_sched_global_edf() {

  int64_t earliest_ddl = 0;
  uint8_t earliest_slot = POK_CONFIG_SCHEDULING_NBSLOTS;
  for (uint8_t slot = 0; slot < POK_CONFIG_SCHEDULING_NBSLOTS; ++slot) {
    if (earliest_slot == POK_CONFIG_SCHEDULING_NBSLOTS ||
        pok_sched_slots_abs_deadline[slot] < earliest_ddl) {
      earliest_ddl = pok_sched_slots_abs_deadline[slot];
      earliest_slot = slot;
    }
  }

  pok_sched_current_slot = earliest_slot;
  pok_sched_next_deadline += pok_sched_slots[pok_sched_current_slot];
...
#ifdef POK_NEEDS_DEBUG
  printf("select slot %d, earlist deadline %lld, number of remaining "
         "slots: %d\n",
         pok_sched_current_slot, pok_sched_slots_abs_deadline[earliest_slot],
         pok_sched_remaining_slot);
#endif

  pok_sched_slots_abs_deadline[earliest_slot] =
      pok_sched_next_major_frame + pok_sched_slots_deadline[earliest_slot];
}
#endif

#ifdef POK_NEEDS_SCHED_GLOBAL_WRR
uint64_t pok_sched_slots_remaining_weight[POK_CONFIG_SCHEDULING_NBSLOTS] =
    (uint64_t[])POK_CONFIG_SCHEDULING_SLOTS;
uint64_t pok_sched_slot_time_slice =
    (uint64_t)POK_CONFIG_SCHEDULING_SLOTS_TIME_SLICE;

void pok_sched_global_wrr() {
  // 每个Major Frame初始化权重
  if (pok_sched_remaining_slot == POK_CONFIG_SCHEDULING_NBSLOTS &&
      pok_sched_slots_remaining_weight[0] == 0) {
    for (uint8_t slot = 0; slot < POK_CONFIG_SCHEDULING_NBSLOTS; ++slot) {
      pok_sched_slots_remaining_weight[slot] = pok_sched_slots[slot];
    }
  }

  uint64_t max_weight = 0;
  uint8_t max_slot = POK_CONFIG_SCHEDULING_NBSLOTS;
  uint8_t slot = pok_sched_current_slot;

  // 遍历寻找下一个slot
  for (slot = 0; slot < POK_CONFIG_SCHEDULING_NBSLOTS; slot += 1) {
    if (max_slot == POK_CONFIG_SCHEDULING_NBSLOTS ||
        pok_sched_slots_remaining_weight[slot] > max_weight) {
      max_weight = pok_sched_slots_remaining_weight[slot];
      max_slot = slot;
    }
  }

  pok_sched_slots_remaining_weight[max_slot] -= pok_sched_slot_time_slice;
  pok_sched_current_slot = max_slot;
  pok_sched_next_deadline += pok_sched_slot_time_slice;

  if (pok_sched_slots_remaining_weight[max_slot] == 0) {
    if (pok_sched_remaining_slot > 1) {
      pok_sched_remaining_slot -= 1;
    } else {
      pok_sched_remaining_slot = POK_CONFIG_SCHEDULING_NBSLOTS;
...
#ifdef POK_NEEDS_DEBUG
  printf("select slot %d, number of remaining slots: %d\n",
         pok_sched_current_slot, pok_sched_remaining_slot);
#endif
}
#endif
#endif
...
uint32_t pok_elect_thread(uint8_t new_partition_id) {
  ...
  for (i = 0; i < new_partition->nthreads; i++) {
    ...
    defined(POK_NEEDS_PORTS_SAMPLING)
      ...
      if ((thread->state == POK_STATE_WAIT_NEXT_ACTIVATION) &&
        ...
        thread->next_deadline = thread->next_activation + thread->deadline;
  ...
  case POK_PARTITION_MODE_NORMAL:
    ...
    elected = new_partition->sched_func(
        new_partition->thread_index_low,
        new_partition->thread_index_low + new_partition->thread_index,
...
uint32_t pok_sched_part_edf(const uint32_t index_low, const uint32_t index_high,
  ...
  uint32_t earliest_thread = POK_CONFIG_NB_THREADS;
  uint8_t current_proc = pok_get_proc_id();
  ...
  do {
    if (pok_threads[i].state == POK_STATE_RUNNABLE &&
        pok_threads[i].processor_affinity == current_proc &&
        (earliest_thread == POK_CONFIG_NB_THREADS ||
         pok_threads[i].next_deadline < earliest_deadline)) {
      earliest_deadline = pok_threads[i].next_deadline;
      earliest_thread = i;
    }
    i++;
    if (i >= index_high) {
  ...
  uint32_t elected =
      earliest_thread != POK_CONFIG_NB_THREADS ? earliest_thread : IDLE_THREAD;
  ...
  if (elected != current_thread &&
    ...
    if (elected == IDLE_THREAD) {
      ...
      printf("--- Scheduling processor: %hhd\n    scheduling thread %d "
             "(deadline "
             "%lld)\n",
             current_proc, elected, pok_threads[elected].next_deadline);
      for (uint32_t i = index_low; i < index_high; i++) {
        if (pok_threads[i].state == POK_STATE_RUNNABLE &&
            pok_threads[i].processor_affinity == current_proc) {
          if (i != elected) {
            printf("%s %d (%lld)", first ? "    other ready: " : ",", i,
                   pok_threads[i].next_deadline);
            first = 0;
          }
        } else {
          non_ready++;
...
// 用于64位整数取模
int64_t do_mod(int64_t x, int64_t y) {
  int64_t i = 0;
  for (i = x; i > 0; i -= y) {
  }
  return i;
...
uint32_t pok_sched_part_wrr(const uint32_t index_low, const uint32_t index_high,
  ...
  uint32_t max_thread = POK_CONFIG_NB_THREADS;
  uint8_t current_proc = pok_get_proc_id();

  if (prev_thread == IDLE_THREAD)
    from = index_low;

  if ((do_mod(pok_threads[current_thread].time_capacity -
                  pok_threads[current_thread].remaining_time_capacity,
              pok_threads[current_thread].time_slice) != 0 ||
  ...
  do {
    ...
    if (pok_threads[i].state == POK_STATE_RUNNABLE &&
        pok_threads[i].processor_affinity == current_proc &&
        (max_weight == -1 ||
         pok_threads[i].remaining_time_capacity > max_weight)) {
      max_weight = pok_threads[i].remaining_time_capacity;
      max_thread = i;
    }
  } while (i != from);

  uint32_t elected =
      max_thread != POK_CONFIG_NB_THREADS ? max_thread : IDLE_THREAD;
  ...
  if (elected != current_thread &&
    ...
    if (elected == IDLE_THREAD) {
      ...
      printf("--- Scheduling processor: %hhd\n    scheduling thread %d "
             ...
             "%d) %lld\n",
             current_proc, elected, pok_threads[elected].priority,
             pok_threads[elected].time_capacity);
      for (uint32_t i = index_low; i < index_high; i++) {
        if (pok_threads[i].state == POK_STATE_RUNNABLE &&
            pok_threads[i].processor_affinity == current_proc) {
          if (i != elected) {
...
#ifdef POK_NEEDS_SCHED_MLFQ
typedef struct {
  uint32_t queue[POK_CONFIG_NB_THREADS];
  int64_t remaining_time[POK_CONFIG_NB_THREADS];
  int64_t time_slice;
  uint32_t begin[POK_CONFIG_NB_PARTITIONS];
  uint32_t end[POK_CONFIG_NB_PARTITIONS];
  uint32_t is_empty[POK_CONFIG_NB_PARTITIONS];
} pok_queue_t;

uint32_t pok_queue_deque(pok_queue_t *q, uint8_t partition_id) {
  uint32_t tid = q->queue[q->begin[partition_id]];
  q->begin[partition_id] =
      pok_partitions[partition_id].thread_index_low +
      (q->begin[partition_id] + 1) % pok_partitions[partition_id].nthreads;
  if (q->begin[partition_id] == q->end[partition_id]) {
    q->is_empty[partition_id] = 1;
  }
  return tid;
}

uint32_t pok_queue_front(pok_queue_t *q, uint8_t partition_id) {
  return q->queue[q->begin[partition_id]];
}

int64_t pok_queue_time_decrease(pok_queue_t *q, uint8_t tid) {
  q->remaining_time[tid] -= 1;
  return q->remaining_time[tid];
}

void pok_queue_append(pok_queue_t *q, uint32_t tid, uint8_t partition_id,
                      int64_t remaining_time) {
  q->queue[q->end[partition_id]] = tid;
  q->remaining_time[tid] = remaining_time;
  q->end[partition_id] =
      pok_partitions[partition_id].thread_index_low +
      (q->end[partition_id] + 1) % pok_partitions[partition_id].nthreads;
  q->is_empty[partition_id] = 0;
}

uint32_t pok_queue_empty(pok_queue_t *q, uint8_t partition_id) {
  return q->is_empty[partition_id];
}

pok_queue_t pok_queue1[POK_CONFIG_NB_PROCESSORS];
pok_queue_t pok_queue2[POK_CONFIG_NB_PROCESSORS];
pok_queue_t pok_queue3[POK_CONFIG_NB_PROCESSORS];
int64_t pok_sched_mlfq_cnt;
const int64_t pok_sched_mlfq_quantum = 15000;

void pok_sched_queue_initialize() {
  for (uint8_t pid = 0; pid < POK_CONFIG_NB_PROCESSORS; ++pid) {
    for (uint8_t i = 0; i < POK_CONFIG_NB_PARTITIONS; ++i) {
      pok_queue1[pid].begin[i] = pok_partitions[i].thread_index_low;
      pok_queue1[pid].end[i] = pok_partitions[i].thread_index_low;
      pok_queue1[pid].time_slice = 100;
      pok_queue1[pid].is_empty[i] = 1;
      pok_queue2[pid].begin[i] = pok_partitions[i].thread_index_low;
      pok_queue2[pid].end[i] = pok_partitions[i].thread_index_low;
      pok_queue2[pid].time_slice = 500;
      pok_queue2[pid].is_empty[i] = 1;
      pok_queue3[pid].begin[i] = pok_partitions[i].thread_index_low;
      pok_queue3[pid].end[i] = pok_partitions[i].thread_index_low;
      pok_queue3[pid].time_slice = 1000;
      pok_queue3[pid].is_empty[i] = 1;
    }
  }
  pok_sched_mlfq_cnt = 0;

#ifdef POK_NEEDS_DEBUG
  printf("POK MLFQ initialized!\n");
#endif
}

uint32_t pok_queue_traverse(pok_queue_t *q) {
  uint32_t elected = IDLE_THREAD;
  uint32_t from = POK_CONFIG_NB_THREADS;
  uint32_t end = q->end[pok_current_partition];
  uint32_t tid = POK_CONFIG_NB_THREADS;

  while (pok_queue_empty(q, pok_current_partition) == 0 && from != end) {
    tid = pok_queue_front(q, pok_current_partition);
    if (pok_threads[tid].state != POK_STATE_RUNNABLE) {
      pok_queue_deque(q, pok_current_partition);
      pok_queue_append(q, tid, pok_current_partition, q->remaining_time[tid]);
    } else {
      elected = tid;
      break;
    }
    from = q->begin[pok_current_partition];
  }
  return elected;
}

void pok_queue_boost(pok_queue_t *q, const uint32_t index_low,
                     const uint32_t index_high) {
  for (uint32_t i = index_low; i < index_high; ++i) {
    pok_queue_append(q, i, pok_current_partition, q->time_slice);
  }
...
void pok_queue_clear(pok_queue_t *q, const uint32_t index_low) {
  q->begin[pok_current_partition] = index_low;
  q->end[pok_current_partition] = index_low;
  q->is_empty[pok_current_partition] = 1;
}
...
uint32_t pok_sched_part_mlfq(const uint32_t index_low,
  ...
  if (pok_sched_mlfq_cnt == 0) {
    pok_queue_clear(&pok_queue1[current_proc], index_low);
    pok_queue_clear(&pok_queue2[current_proc], index_low);
    pok_queue_clear(&pok_queue3[current_proc], index_low);

    pok_queue_boost(&pok_queue1[current_proc], index_low, index_high);
  }

  pok_sched_mlfq_cnt += 1;
  if (pok_sched_mlfq_cnt == pok_sched_mlfq_quantum) {
    pok_sched_mlfq_cnt = 0;
  }

  elected = pok_queue_traverse(&pok_queue1[current_proc]);
  if (elected != IDLE_THREAD) {
    int64_t remaining_time =
        pok_queue_time_decrease(&pok_queue1[current_proc], elected);
    if (remaining_time == 0) {
      pok_queue_deque(&pok_queue1[current_proc], pok_current_partition);
      pok_queue_append(&pok_queue2[current_proc], elected,
                       pok_current_partition,
                       pok_queue2[current_proc].time_slice);
    }
  } else {
    elected = pok_queue_traverse(&pok_queue2[current_proc]);
    if (elected != IDLE_THREAD) {
      int64_t remaining_time =
          pok_queue_time_decrease(&pok_queue2[current_proc], elected);
      if (remaining_time == 0) {
        pok_queue_deque(&pok_queue2[current_proc], pok_current_partition);
        pok_queue_append(&pok_queue3[current_proc], elected,
                         pok_current_partition,
                         pok_queue3[current_proc].time_slice);
      }

    } else {
      elected = pok_queue_traverse(&pok_queue3[current_proc]);
      if (elected != IDLE_THREAD) {
        int64_t remaining_time =
            pok_queue_time_decrease(&pok_queue3[current_proc], elected);
        if (remaining_time == 0) {
          pok_queue_deque(&pok_queue3[current_proc], pok_current_partition);
        }
      }
    }
  }
  ...
  if (elected != current_thread &&
    ...
    if (elected == IDLE_THREAD) {
      ...
      uint32_t first = 1;
      printf("--- Scheduling processor: %hhd\n  sched_cnt: %lld  scheduling "
             "thread %d "
             "(priority "
             "%d)\n",
             current_proc, pok_sched_mlfq_cnt, elected,

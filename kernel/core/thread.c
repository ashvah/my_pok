...
void pok_idle_thread_init() {
  ...
  for (int i = 0; i < POK_CONFIG_NB_PROCESSORS; i++) {
    ...
    pok_threads[IDLE_THREAD - i].next_deadline = 0;
    ...
    pok_threads[IDLE_THREAD - i].time_slice = 0;
...
void pok_thread_init(void) {
  ...
  for (i = 0; i < POK_CONFIG_NB_THREADS; ++i) {
    ...
    pok_threads[i].next_deadline = 0;
    pok_threads[i].time_slice = 1;
...
pok_ret_t pok_partition_thread_create(uint32_t *thread_id,
  ...
  if (attr->deadline > 0) {
    ...
    pok_threads[id].next_deadline = attr->deadline;
  }
  ...
  if (attr->time_slice > 0) {
    pok_threads[id].time_slice = attr->time_slice;
  }
